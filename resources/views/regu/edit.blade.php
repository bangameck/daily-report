<x-app-layout>
    <x-slot name="header">
        Edit Regu: {{ $regu->nm_regu }}
    </x-slot>

    <div x-data="reguForm({
        nama: '{{ $regu->nm_regu }}',
        foto: '{{ $regu->f_regu ? Storage::url($regu->f_regu) : '' }}'
    })">
        ...
        <div x-show="!isLoading" x-transition>
            <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 px-4 py-4">
                ...
                <form @submit.prevent="submitForm" action="{{ route('regu.update', $regu) }}" method="POST"
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-6 space-y-6">
                        ...
                        <x-forms.floating-input name="nm_regu" label="Nama Regu" :value="old('nm_regu', $regu->nm_regu)" required />
                        ...
                        <x-forms.floating-select name="karu" label="Kepala Regu (Karu) - (Opsional)">
                            ...
                            @foreach ($karuUsers as $karu)
                                <option value="{{ $karu->id }}" @selected(old('karu', $regu->karu) == $karu->id)>
                        ...
                        <div class="flex justify-end pt-4">
                            ...
                            <button type="submit" :disabled="formLoading"
                                ...
                                <span x-show="!formLoading">Update Regu</span>
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('reguForm', (regu) => ({
                    isLoading: true,
                    formLoading: false,
                    imagePreview: regu.foto ||
                        `https://ui-avatars.com/api/?name=${encodeURIComponent(regu.nama)}&color=FFFFFF&background=04449c&size=128`,
