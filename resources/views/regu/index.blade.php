            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-4 py-4">
                ...
                <div class="mb-4 p-4 bg-white rounded-lg shadow-sm">
                    <div class="flex flex-col sm:flex-row justify-between sm:items-center">
                        <form action="{{ route('regu.index') }}" method="GET" class="w-full sm:w-1/2">
                            <label for="search-input" class="sr-only">Cari regu...</label>
                            <div class="relative">
                                ...
                                <input type="text" name="search" id="search-input" value="{{ $search ?? '' }}"
                                    ...
                                    placeholder="Cari regu atau nama karu...">
                            </div>
                        </form>
                        <a href="{{ route('regu.create') }}"
                            ...
                            <div class="p-4 flex items-center justify-between space-x-4">
                                <div class="flex items-center space-x-3">
                                    <img class="h-12 w-12 rounded-full object-cover"
                                        src="{{ $regu->f_regu ? Storage::url($regu->f_regu) : 'https://ui-avatars.com/api/?name=' . urlencode($regu->nm_regu) . '&color=FFFFFF&background=04449c' }}"
                                        alt="{{ $regu->nm_regu }}">
                                    <div class="flex-1">
                                        <p class="text-sm font-bold text-sky-900">{{ $regu->nm_regu }}</p>
                                        <p class="text-xs text-gray-600">
                                            Karu: <span
                                                class="font-medium">{{ $regu->kepalaRegu->nama ?? 'Belum Ditentukan' }}</span>
                                        </p>
                                    </div>
                                </div>
                                ...
                                <x-dropdown align="right" width="48">
                                    ...
                                    <x-slot name="content">
                                        <x-dropdown-link :href="route('regu.edit', $regu)">
                                            Edit Regu
                                        </x-dropdown-link>

                                        <form method="POST" action="{{ route('regu.destroy', $regu) }}"
                                            id="delete-form-{{ $regu->id_regu }}">
                                            @csrf
                                            @method('DELETE')
                                            <a href="{{ route('regu.destroy', $regu) }}"
                                                class="delete-button block w-full px-4 py-2 text-left text-sm text-red-600 hover:bg-red-50"
                                                data-form-id="delete-form-{{ $regu->id_regu }}">
                                                Hapus Regu
                                            </a>
                                        </form>
                                    </x-slot>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                document.querySelectorAll('.delete-button').forEach(button => {
                    button.addEventListener('click', function(e) {
                        ...
                        Swal.fire({
                            title: 'Yakin mau hapus regu?',
                            text: "Anggota regu ini tidak akan terhapus (hanya relasinya).",
                            icon: 'warning',
