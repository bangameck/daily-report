<x-app-layout>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-4 py-4">
                ...
                <div class="mb-4 p-4 bg-white rounded-lg shadow-sm">
                    <div class="flex flex-col sm:flex-row justify-between sm:items-center">
                        <form action="{{ route('users.index') }}" method="GET" class="w-full sm:w-1/2">
                            <label for="search-input" class="sr-only">Cari user...</label>
                            <div class="relative">
                                ...
                                <input type="text" name="search" id="search-input" value="{{ $search ?? '' }}"
                                    ...
                                    placeholder="Cari user (nama, username, NIK...)">
                            </div>
                        </form>
                            ...
                            <div class="p-4 flex items-center justify-between space-x-4">
                                <div class="flex items-center space-x-3">
                                    <img class="h-12 w-12 rounded-full object-cover"
                                        src="{{ $user->f_ust ? Storage::url($user->f_ust) : 'https://ui-avatars.com/api/?name=' . urlencode($user->nama) . '&color=FFFFFF&background=04449c' }}"
                                        alt="{{ $user->nama }}">
                                    <div class="flex-1">
                                        <a href="{{ route('users.show', $user) }}"
                                            class="text-sm font-bold text-sky-900 hover:underline">
                                            {{ $user->nama }}
                                        </a>
                                        <p class="text-xs text-gray-600">{{ $user->username }} |
                                            {{ $user->role->nm_role }}</p>
                                        <p class="text-xs text-gray-500">{{ $user->regu->nm_regu ?? 'Tanpa Regu' }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center space-x-3">
                                    <form action="{{ route('users.toggleStatus', $user) }}" method="POST">
                                        ...
                                        @method('PATCH')
                                        <button type="submit"
                                            title="{{ $user->status ? 'Klik untuk Non-aktifkan' : 'Klik untuk Aktifkan' }}"
                                            class="rounded-full px-2 py-0.5 text-xs font-semibold {{ $user->status ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    ...
                                    <x-dropdown align="right" width="48">
                                        ...
                                        <x-slot name="content">
                                            ...
                                            <form method="POST" action="{{ route('users.destroy', $user) }}"
                                                ...
                                                @csrf
                                                @method('DELETE')
                                                <!-- [FIX] Ganti class dari 'logout-button' menjadi 'delete-button' -->
    ...
    <!-- [BARU] Skrip SweetAlert khusus untuk Hapus -->
    @push('scripts')
        <script>
